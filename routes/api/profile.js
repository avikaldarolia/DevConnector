...
router.get("/me", auth, async (req, res) => {
  try {
    ...
    ).populate("user", ["name", "avatar"]); //for this we made changes in our profile schema under user tag. Also dounts about this req.user.id
...
router.get("/user/:user_id", async (req, res) => {
  try {
    ...
    }).populate("user", ["name", "avatar"]); //in profiles we dont get the name and the avatar. So we use this populate func where 1st argument is from where we are taking what we want to populate, and 2nd argument is the array of items that we want to populate
    if (!profile) {
      return res
        .status(400)
